...
## 7. 斩杀评分逻辑错误 (Lethal Scoring Bug)
*   **现象**: 当场上仅有1个怪物（11血），手牌中有2张“打击”（共12伤）时，AI 推荐打出“防御”而非“打击”。
*   **原因**: 原始评分逻辑仅计算单张卡牌是否能造成斩杀（Single Card Lethal），忽略了多张卡牌组合造成的斩杀（Combo Lethal）。由于“防御”牌通常具有较高的基础分（保命），在未识别出斩杀机会时，其得分高于普通的“打击”。
*   **解决**:
    1. 在 `calculate_recommendation` 中预先计算当前手牌的总伤害 (`total_hand_damage`)。
    2. 判断总伤害是否大于等于怪物生命值 (`combo_lethal`)。
    3. 如果满足组合斩杀条件，为所有参与攻击的卡牌增加额外的斩杀奖励分 (40分)，使其总分超过“防御”牌。
